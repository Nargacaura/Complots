...
signal add_card(card, card_index)
"""
Signal: add_card
...
func add_card(card: Card) -> void:
	...
	emit_signal("add_card", card, _hand)
...
func remove_card(index: int) -> void:
	...
	_hand.remove(index)
	emit_signal("remove_card", index, _hand)
...
func kill_card(card_index: int) -> void:
	...
	emit_signal("kill_card", card_index, get_hand()[card_index].get_type(), is_alive(), _hand)

func select_cards(_cards: Array, _qty: int = 1, _text: String = "", _select_type: int = Action.SELECT_TYPE.KEEP) -> void:
	"""
	Method: select_cards
...
func branch_options(_options: Array, _text: String, _card_type: int) -> void:
	"""
...
func request_kill_card(_text: String = "", _qty: int = 1, _select_type: int = Action.SELECT_TYPE.KILL) -> void:
	"""
...
# Signals Functions
#------------------------------------------------------------------------------#
func _on_start_reaction(_calling_action: Action, _time: int) -> void:
	"""

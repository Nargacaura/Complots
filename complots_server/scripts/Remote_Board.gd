...
func _on_make_reaction(calling_action: Action, roles: Array) -> void:
	...
	_players[AppManager.player_game_id - 1]._on_start_reaction(calling_action)


func _on_start_reaction(_calling_action: Action, _time: int) -> void:
	# Send from the board not the player
	# TODO
	pass


func _on_choose_cards(cards: Array, qty: int, text: String) -> void:
	_players[AppManager.player_game_id - 1].select_cards(cards, qty, text, Action.SELECT_TYPE.KILL)


func _on_choose_options(options: Array, text: String) -> void:
	var card_type: int = 2 # TODO: Change this, card_type must come from the arguments

...
signal update_history(message, action)
"""
...
:param Action action: Action related.
"""
...
signal start_reaction(action, time)
"""
...
signal update_player_view(player)
...
func run() -> void:
	...
	# Game Loop
	while !is_game_over():
		_game_state = GAME_STATE.BEGIN_TURN
		emit_signal("begin_turn", _active_player_id)
		...
		_game_state = GAME_STATE.PLAYER_TURN
		...
		if !played_action._can_be_countered and !played_action._can_be_doubted:
			...
			_game_state = GAME_STATE.REACTION
...
func _deal_cards_two_players() -> void:
	...
	for player in _players:
		...
		player.select_cards(cards, 1, "Draw one of the following cards", Action.SELECT_TYPE.KEEP)
...
func get_winning_player() -> Player_Base:
	for player in _players:
		if player.is_alive():
			return player
	return null
...
func _emit_action(action: Action) -> void:
	...
	emit_signal("update_history", get_action_message(action), action)
	...
	if action.get_action_type() == Action.ACTION_TYPE.DOUBT:
		...
		emit_signal("start_reaction", action, Action.ACTION_SHORT_TIMEOUT if !action._can_be_countered and !action._can_be_doubted else Action.ACTION_TIMEOUT)
...
func _is_action_valid(action: Action) -> bool:
	...
	if !_set_and_check_action_attributes(action):
		return false
	if Action.is_main_action(action) and _game_state != GAME_STATE.PLAYER_TURN:
		return false
	if Action.is_reaction(action) and _game_state != GAME_STATE.REACTION:
		return false
...
func _resolve_action() -> void:
	...
	match action.get_action_type():
		...
		Action.ACTION_TYPE.COUNTER:
			...
			_actions.pop_back()
			emit_signal("update_history", "%s's counter was successful. Action '[color=#95a5a6]%s[/color]' was canceled." % [sender.get_bbusername(), Action.get_action_string(countered_action.get_action_type())], action)
		#----------------------------------------------------------------------#
		# RESOLVE INCOME
		#----------------------------------------------------------------------#
		...
		Action.ACTION_TYPE.FOREIGN_AID:
			...
			emit_signal("update_history", "%s took 2 coins to the treasure." % [sender.get_bbusername()], action)
		#----------------------------------------------------------------------#
		# RESOLVE COUP
		#----------------------------------------------------------------------#
		Action.ACTION_TYPE.COUP:
		...
		Action.ACTION_TYPE.DUKE:
			...
			emit_signal("update_history", "%s took 3 coins to the treasure." % [sender.get_bbusername()], action)
		#----------------------------------------------------------------------#
		# RESOLVE ASSASSIN
		#----------------------------------------------------------------------#
		Action.ACTION_TYPE.ASSASSIN:
		...
		Action.ACTION_TYPE.CAPTAIN:
			...
			emit_signal("update_history", "%s stole %d %s from %s." % [sender.get_bbusername(), action.get_cost(), "coins" if action.get_cost() > 1 else "coin", target.get_bbusername()], action)
		#----------------------------------------------------------------------#
		# RESOLVE AMBASSADOR
		#----------------------------------------------------------------------#
		Action.ACTION_TYPE.AMBASSADOR:
			...
			_start_timer(_action_timer, Action.ACTION_TIMEOUT, "_on_choice_timeout")
			sender.call_deferred("select_cards", cards, 2, "Select two cards to return to the Court.", Action.SELECT_TYPE.RETURN_TO_COURT)
			...
			emit_signal("update_player_view", sender)
		#----------------------------------------------------------------------#
		...
		Action.ACTION_TYPE.INQUISITOR_1:
			...
			for card in sender.get_hand():
				if !card.is_dead():
					cards.push_back(card)
			_start_timer(_action_timer, Action.ACTION_TIMEOUT, "_on_choice_timeout")
			sender.call_deferred("select_cards", cards, 1, "Select a card to return to the Court.", Action.SELECT_TYPE.RETURN_TO_COURT)
			choice = yield(self, "player_card_choice_valid")
			_stop_timer(_action_timer, "_on_choice_timeout")
			# If the player asks to reshuffle the card they've just drawn...
			if choice[0] < 1:
		...
		Action.ACTION_TYPE.INQUISITOR_2:
			...
			_start_timer(_action_timer, Action.ACTION_TIMEOUT, "_on_choice_timeout")
			target.call_deferred("select_cards", cards, 1, "Select 1 card to show to %s's Inquisitor" % [sender.get_username()], Action.SELECT_TYPE.SHOW)
			choice = yield(self, "player_card_choice_valid")
			_stop_timer(_action_timer, "_on_choice_timeout")
			...
			sender.call_deferred("branch_options", options, text, card.get_type())
			if yield(self,"player_option_choice_valid")[0] == 1:
				...
				yield(target,"hand_updated")
				emit_signal("update_history", "%s made %s exchange one card with the Court." % [sender.get_bbusername(), target.get_bbusername()], action)
			else:
				emit_signal("update_history", "%s let %s keep his card." % [sender.get_bbusername(), target.get_bbusername()], action)
...
func kill_procedure(player: Player_Base, ask_message: String, action_message: String) -> void:
	...
	player.call_deferred("request_kill_card", ask_message, 1, Action.SELECT_TYPE.KILL)
...
func kill_player_card(player: Player_Base, victim: Array, message: String) -> void:
	...
	emit_signal("update_history", message, null)

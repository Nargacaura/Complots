...
signal make_reaction(actions, roles, player)
"""
make_reaction
...
:param Array roles: Array containing the list of roles allowed in this game.
:param Action player: This player object.
"""

signal choose_cards(cards, qty, text, selection_type)
"""
choose_cards
...
func select_cards(cards: Array, qty: int = 1, text: String = "", select_type: int = Action.SELECT_TYPE.KEEP) -> void:
	"""
	select_cards
...
func branch_options(options: Array, text: String, card_type: int) -> void:
	"""
	branch_options
...
func request_kill_card(text: String = "", qty: int = 1, select_type: int = Action.SELECT_TYPE.KILL) -> void:
	"""
	request_kill_card
...
func _on_start_reaction(calling_action: Action, _time: int) -> void:
	...
	if !calling_action._can_be_countered and !calling_action._can_be_doubted:
		return
	emit_signal("make_reaction", calling_action, _roles, self)
	yield(self, "resume")
...
func connect_signals_core_to_view(view: Node) -> void:
	"""
	connect_signals

...
const Card_Scene = preload("res://scenes/debug/Card_Test.tscn")
...
func _on_change_balance(_id: int, balance: int):
	$Player_Badge/Info/Coins.set_text("Coins: " + str(balance))
	emit_signal("change_balance", _id, balance)
...
func _on_play_turn(player: Player_Base, roles: Array, players_data: Dictionary) -> void:
	...
	if player.get_balance() >= 7:
		...
		if player.get_balance() >= 10:
			...
			label.text = "You have more than 10 coins in your income. Better command a Coup out of them!"
	...
	for player_id in _players_data["alive"].keys():
		if player_id != _id:
			if action_type == Action.ACTION_TYPE.CAPTAIN:
				...
				_create_select_button("%s" % _players_data["alive"][player_id]["username"], player_id, _target_container.get_child(1), "_on_target_pressed", [button_id, min_targets, max_targets])
				button_id += 1
	_target_container.get_child(0).text = 'Potential targets for "%s"' % Action.get_action_string(action_type)
...
func _on_kill_card(card_id: int, card_type: int, is_alive: bool = true, id: int = -1) -> void:
	if _id != id:
		return

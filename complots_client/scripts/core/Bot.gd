		...
		if player.get_id() != get_id():
			_game_data["players"][player.get_id()] = {
				...
				"hand": [
					...
					{"card_type": 0, "is_dead": player.get_hand()[1].is_dead()}
				],
				"announcement": {0: 0}
			}
			for role in _roles:
...
func add_card(card: Card) -> void:
	...
	"""
	print("add_card")
	emit_signal("add_card", Card.new(Card.CARD_TYPE.HIDDEN))
...
func select_cards(cards: Array, qty: int = 1, _text: String = "") -> void:
	"""
	Method: select_cards
	...
	match _bot_difficulty:
		BOT_DIFFICULTY.EASY:
			selection = select_cards_easy(cards, qty)
		BOT_DIFFICULTY.MEDIUM:
			selection = select_cards_medium(cards, qty)
		BOT_DIFFICULTY.HARD:
			selection = select_cards_hard(cards, qty)
		_:
			selection = select_cards_easy(cards, qty)
	emit_signal("player_card_choice", selection)

func branch_options(options: Array, _text: String) -> void:
	"""
	...
	match _bot_difficulty:
		BOT_DIFFICULTY.EASY:
			value = branch_options_easy(options)
		BOT_DIFFICULTY.MEDIUM:
			value = branch_options_medium(options)
		BOT_DIFFICULTY.HARD:
			value = branch_options_hard(options)
		_:
			value = branch_options_easy(options)
	emit_signal("player_option_choice", value)

func request_kill_card(_text: String = "", qty: int = 1) -> void:
	"""
...
#------------------------------------------------------------------------------#
# TODO: Make AI decision here
func select_cards_easy(cards: Array, qty: int) -> Array:
	"""
...
# TODO: Make AI decision here
func select_cards_medium(cards: Array, qty: int) -> Array:
	"""
	...
	:return Array: An array containing the selected cards index.
	"""
	return select_cards_easy(cards, qty)

# TODO: Make AI decision here
func select_cards_hard(cards: Array, qty: int) -> Array:
	"""
...
#------------------------------------------------------------------------------#
# TODO: Make AI decision here
func branch_options_easy(options: Array) -> int:
	"""
...
# TODO: Make AI decision here
func branch_options_medium(options: Array) -> int:
	"""
	...
	:return int: The selected option index.
	"""
	return branch_options_easy(options)

# TODO: Make AI decision here
func branch_options_hard(options: Array) -> int:
	"""
